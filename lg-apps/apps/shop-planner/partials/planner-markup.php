<?php
/**
 * The Roadman Shop Planner — modal markup, WordPress-free.
 *
 * WHY THIS IS A PARTIAL AND NOT INLINE IN app.php. /shop-layout-planner/ is now
 * served by a standalone front controller (webroot/shop-layout-planner.php) that
 * does not render through the WordPress theme. Both that controller AND the
 * plugin's wp_footer render need this exact markup, and a COPY is how the two
 * renders start disagreeing three months from now. So it lives here once.
 *
 * The body below is lifted VERBATIM from lgapps_shop_planner_render_modal()
 * (app.php, pre-extraction). It contains no WordPress at all — no esc_*, no
 * get_*, no is_user_logged_in(), no translation calls — which is precisely what
 * makes serving it outside WordPress possible.
 *
 * Guard: LGAPPS_PARTIAL is deliberately NOT the usual `defined('ABSPATH')`.
 * This file must be includable by the standalone controller in the degraded case
 * where WordPress could not be loaded at all, so it cannot demand ABSPATH; but it
 * still must not render anything if fetched directly over HTTP.
 */
if ( ! defined( 'LGAPPS_PARTIAL' ) ) exit;
?>
