-- ============================================================
-- CANARY 4 of 6 — DRY RUN, IAN ONLY.  READ ONLY.
-- Writes nothing, changes nothing. Safe to run any number of times.
--
-- This is the IAN-ONLY canary set (files 4/5/6). It restores ONLY the
-- 135 connections that touch Ian (WP user 1). The full 746-row set is
-- files 1/2/3 — do not mix them up.
-- ============================================================
...
DO $$ DECLARE n int; BEGIN
  IF to_regclass('public.connections_restore_20260727_ian') IS NULL THEN
    RAISE NOTICE 'canary tag table: DOES NOT EXIST YET (canary has not been applied)';
  ELSE
    EXECUTE 'SELECT count(*) FROM public.connections_restore_20260727_ian' INTO n;
    RAISE NOTICE 'canary tag table: EXISTS, % rows tagged (canary IS applied)', n;
  END IF;
END $$;
...
SELECT 'rows in this canary'             AS what, (SELECT count(*) FROM payload)::text AS n
...
UNION ALL SELECT 'UUID MATCH (must say t)',      ((SELECT uuid FROM ian) = 'f20ad778-1e5e-5508-853b-ad928c499f2f'::uuid)::text
...
UNION ALL SELECT '    pending Ian SENT (outbound, shows on THEIR side)',
                                                 (SELECT count(*) FROM insertable WHERE status='pending' AND requester_uuid='f20ad778-1e5e-5508-853b-ad928c499f2f'::uuid)::text
UNION ALL SELECT '    pending Ian RECEIVED (inbound, shows on HIS side)',
                                                 (SELECT count(*) FROM insertable WHERE status='pending' AND addressee_uuid='f20ad778-1e5e-5508-853b-ad928c499f2f'::uuid)::text
UNION ALL SELECT 'other members whose incoming-request badge will +1',
                                                 (SELECT count(DISTINCT addressee_uuid) FROM insertable WHERE status='pending' AND requester_uuid='f20ad778-1e5e-5508-853b-ad928c499f2f'::uuid)::text
