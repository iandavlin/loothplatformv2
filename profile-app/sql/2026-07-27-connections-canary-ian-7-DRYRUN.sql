-- ============================================================
-- CANARY 7 of 9 — DRY RUN, IAN ONLY, ACCEPTED ONLY.  READ ONLY.
-- Writes nothing, changes nothing. Safe to run any number of times.
--
-- THE SILENT SET. This restores ONLY the 83 ACCEPTED connections touching Ian
-- (WP user 1). It deliberately EXCLUDES his 52 pending requests, because those
-- are all outbound and would place an incoming-request badge on 52 real members
-- tonight for requests Ian sent in 2023-2026. Ian's ruling, 2026-07-28.
--
-- THREE SETS EXIST. Do not mix them up:
--   files 1/2/3  full restore, 746 rows, 302 members
--   files 4/5/6  Ian only, 135 rows (83 accepted + 52 pending)
--   files 7/8/9  THIS ONE — Ian only, 83 accepted, nobody is notified
-- ============================================================
\set ON_ERROR_STOP on

-- Tag-table status. Dynamic, because on the FIRST run this table does not yet
-- exist and a direct reference would fail to parse.
DO $$ DECLARE n int; BEGIN
  IF to_regclass('public.connections_restore_20260727_ian_acc') IS NULL THEN
    RAISE NOTICE 'accepted-canary tag table: DOES NOT EXIST YET (this set has not been applied)';
  ELSE
    EXECUTE 'SELECT count(*) FROM public.connections_restore_20260727_ian_acc' INTO n;
    RAISE NOTICE 'accepted-canary tag table: EXISTS, % rows tagged (this set IS applied)', n;
  END IF;
END $$;
...
SELECT 'rows in this accepted-only canary' AS what, (SELECT count(*) FROM payload)::text AS n
UNION ALL SELECT 'every row is accepted (must say true)',
                                            ((SELECT count(*) FROM payload WHERE status<>'accepted')=0)::text
UNION ALL SELECT 'bridge says Ian uuid is',  (SELECT uuid FROM ian)::text
UNION ALL SELECT 'payload was built for uuid','f20ad778-1e5e-5508-853b-ad928c499f2f'
...
UNION ALL SELECT 'pending rows in this set (must be 0)',
                                            (SELECT count(*) FROM insertable WHERE status<>'accepted')::text
...
UNION ALL SELECT '  (accepted rows raise no badge: requests_pending counts only'      , ''
UNION ALL SELECT '   status=pending. No notification row is written by SQL - only'    , ''
UNION ALL SELECT '   Notifications::push() in PHP does that. No per-event email'      , ''
UNION ALL SELECT '   exists at all. The one visible change is that the connection'    , ''
UNION ALL SELECT '   appears in both members lists and their counts - the point.)'    , ''
UNION ALL SELECT '--- Ian BEFORE ---', ''
...
UNION ALL SELECT 'Ian pending IN now', (SELECT count(*) FROM public.connections c WHERE c.status='pending'  AND c.addressee_uuid=(SELECT uuid FROM ian))::text
...
UNION ALL SELECT '  (pending counts are shown only so you can confirm this set'       , ''
UNION ALL SELECT '   leaves them UNCHANGED. It touches no pending row.)'              , '';
