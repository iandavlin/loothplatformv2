-- ============================================================
-- 2 of 3 — APPLY.  Restores the connections the cutover backfill skipped.
--
-- Idempotent: guarded by the REAL uniqueness constraint on this table,
--   connections_requester_uuid_addressee_uuid_key  UNIQUE (requester_uuid, addressee_uuid)
-- (verified on live, not assumed) PLUS a NOT EXISTS reverse guard, because
-- that constraint is DIRECTIONAL and cannot see an existing reverse row.
-- Running this twice inserts nothing the second time.
--
-- Every inserted row is TAGGED in connections_restore_20260727 so the
-- rollback deletes exactly these rows by primary key and nothing else.
--
-- Original created_at from the legacy row is preserved (dates are real
-- history, back to 2023). Original accepted/pending status is preserved.
-- ============================================================
\set ON_ERROR_STOP on
BEGIN;

CREATE TABLE IF NOT EXISTS public.connections_restore_20260727 (
...
DO $$ DECLARE n int; BEGIN
  ...
  IF n <> 746 THEN RAISE EXCEPTION 'payload is % rows, expected 746 - file is truncated, ABORTING', n; END IF;
END $$;
...
INSERT INTO public.connections_restore_20260727 (connection_id) SELECT id FROM ins;

SELECT 'rows inserted and tagged' AS what, count(*)::text AS n FROM public.connections_restore_20260727;
