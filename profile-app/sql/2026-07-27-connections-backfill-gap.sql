-- =====================================================================
-- Connections backfill — repair of the 2026-06 cutover gap
-- Generated 2026-07-27 from LIVE READ-ONLY snapshot:
--   source A: looth_import.wp_bp_friends  (11,363 rows)
--   source B: profile_app.public.connections (10,862 rows)
--   source C: profile_app.public.wp_user_bridge (1,847 rows)
-- Inserts 750 legacy friendships that have NO counterpart in either
-- direction in public.connections. 748 of them involve a user added to
-- wp_user_bridge AFTER the cutover backfill ran (the re-run that
-- tools/backfill-bb-connections.sh asks for in its own header never
-- happened); 2 are an unexplained residual, both legitimately absent.
--
-- Conventions match the original backfill exactly:
--   is_confirmed=1 -> 'accepted', else 'pending'
--   initiator      -> requester_uuid
--   date_created   -> created_at AND updated_at, naive value read as UTC
--
-- DIFFERENCE FROM THE ORIGINAL SCRIPT (deliberate):
--   the original relied only on ON CONFLICT (requester_uuid, addressee_uuid),
--   which is DIRECTIONAL and therefore cannot see an existing reverse row.
--   That is why live already carries 10 bidirectional duplicate rows.
--   This script adds a NOT EXISTS reverse guard, so it will not add more.
--
-- ROLLBACK IS STATED IN THE .md — read it before running this.
-- =====================================================================
...
SELECT status, count(*) AS before_count FROM public.connections GROUP BY status ORDER BY status;

CREATE TEMP TABLE backfill_gap (
  requester_uuid uuid,
  addressee_uuid uuid,
  status         text,
...
INSERT INTO backfill_gap (requester_uuid, addressee_uuid, status, created_at) VALUES
  ...
  ('f20ad778-1e5e-5508-853b-ad928c499f2f'::uuid, '3ee134f8-8a25-5dc9-a755-8f5e4c2f5bcc'::uuid, 'pending', TIMESTAMPTZ '2025-09-13 16:06:23+00'),
  ('502849b6-cccb-5e29-b1b2-1691436e3c4d'::uuid, '3ee134f8-8a25-5dc9-a755-8f5e4c2f5bcc'::uuid, 'pending', TIMESTAMPTZ '2025-09-22 15:05:54+00'),
  ...
  ('803d6ead-3a10-55b1-8529-b480718147e7'::uuid, '2065b7e1-2a58-5ba4-bab7-76d24380687b'::uuid, 'pending', TIMESTAMPTZ '2026-03-13 16:56:47+00'),
  ...
  ('7cab3bbe-bfda-52f5-9af6-f5cac0cd4b7b'::uuid, '2065b7e1-2a58-5ba4-bab7-76d24380687b'::uuid, 'pending', TIMESTAMPTZ '2026-04-25 19:27:03+00'),
...
-- sanity: the staging set must be exactly 750 rows
DO $$
DECLARE n int;
BEGIN
  SELECT count(*) INTO n FROM backfill_gap;
  IF n <> 750 THEN RAISE EXCEPTION 'staging row count is %, expected 750', n; END IF;
END $$;

INSERT INTO public.connections (requester_uuid, addressee_uuid, status, created_at, updated_at)
SELECT g.requester_uuid, g.addressee_uuid, g.status, g.created_at, g.created_at
FROM backfill_gap g
WHERE g.requester_uuid <> g.addressee_uuid
  AND NOT EXISTS (
        SELECT 1 FROM public.connections c
        WHERE c.requester_uuid = g.addressee_uuid
          AND c.addressee_uuid = g.requester_uuid)
ON CONFLICT (requester_uuid, addressee_uuid) DO NOTHING;

SELECT status, count(*) AS after_count FROM public.connections GROUP BY status ORDER BY status;

-- Ian's own totals, resolved through the bridge from WP user 1 (NOT by display_name)
SELECT c.status, count(*) AS ian_after
FROM public.connections c
JOIN public.users u ON u.uuid IN (c.requester_uuid, c.addressee_uuid)
JOIN public.wp_user_bridge b ON b.user_id = u.id
WHERE b.wp_user_id = 1
