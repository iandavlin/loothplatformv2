-- ============================================================
-- 13 of 15 -- DRY RUN.  READ ONLY.  Writes nothing, changes nothing.
--
...
DO $$ DECLARE n int; BEGIN
  IF to_regclass('public.connections_restore_20260728_all_acc') IS NULL THEN
    RAISE NOTICE 'tag table: DOES NOT EXIST YET (this restore has not been applied)';
  ELSE
    EXECUTE 'SELECT count(*) FROM public.connections_restore_20260728_all_acc' INTO n;
    RAISE NOTICE 'tag table: EXISTS, % rows tagged (this restore IS applied)', n;
  END IF;
END $$;
...
-- a pair already covered in EITHER direction is skipped by the guards in file 14
present_same AS (
  SELECT p.* FROM payload p JOIN public.connections c
    ON c.requester_uuid=p.requester_uuid AND c.addressee_uuid=p.addressee_uuid),
present_opp AS (
  SELECT p.* FROM payload p JOIN public.connections c
    ON c.requester_uuid=p.addressee_uuid AND c.addressee_uuid=p.requester_uuid),
willins AS (
...
allu AS (SELECT requester_uuid AS u FROM payload UNION SELECT addressee_uuid FROM payload),
...
UNION ALL SELECT 'every row is accepted',            (SELECT (count(*) FILTER (WHERE status<>'accepted'))=0 FROM payload)::text
UNION ALL SELECT 'members affected',                 (SELECT count(*) FROM allu)::text
UNION ALL SELECT '--- IS THIS THE RIGHT DATABASE ---', ''
...
UNION ALL SELECT 'payload uuids NOT found in users', (SELECT count(*) FROM allu WHERE u NOT IN (SELECT uuid FROM public.users))::text
UNION ALL SELECT '  (both must be clean. The uuid check alone does NOT tell dev2'  , ''
UNION ALL SELECT '   from live -- uuids are deterministic, so WP user 1 resolves'  , ''
UNION ALL SELECT '   the same on both. The missing-uuid count is the real'        , ''
UNION ALL SELECT '   discriminator: this payload was built from live.)'           , ''
UNION ALL SELECT '--- WHAT WILL HAPPEN ---', ''
UNION ALL SELECT 'WILL INSERT (after guards)',       (SELECT count(*) FROM willins)::text
UNION ALL SELECT 'already present, same direction',  (SELECT count(*) FROM present_same)::text
UNION ALL SELECT 'already present, OPPOSITE dir',    (SELECT count(*) FROM present_opp)::text
UNION ALL SELECT '  (both are SKIPPED, not duplicated. Same-direction is stopped'  , ''
UNION ALL SELECT '   by UNIQUE(requester_uuid,addressee_uuid); opposite-direction' , ''
UNION ALL SELECT '   by an explicit NOT EXISTS reverse guard, because that'        , ''
UNION ALL SELECT '   constraint is DIRECTIONAL and would NOT catch it.)'           , ''
UNION ALL SELECT '--- WHO GETS NOTIFIED ---', ''
UNION ALL SELECT 'members who get an incoming-request badge', '0'
UNION ALL SELECT 'members who get a bell notification',       '0'
UNION ALL SELECT 'members who get an email',                  '0'
UNION ALL SELECT 'pending rows created by this file',
                                                     (SELECT count(*) FROM payload WHERE status<>'accepted')::text
UNION ALL SELECT '  (ZERO, and here is the mechanism. The friends badge counts'    , ''
UNION ALL SELECT '   status=pending only, and every row here is accepted, so no'   , ''
UNION ALL SELECT '   badge moves. A bell is a row in notifications, written only'  , ''
UNION ALL SELECT '   by Notifications::push() in PHP -- a raw SQL INSERT mints'    , ''
UNION ALL SELECT '   none. And no per-event email exists at all; the weekly'       , ''
UNION ALL SELECT '   digest reads notifications, which stays empty of these.'      , ''
UNION ALL SELECT '   Verified on live: connections carries exactly one trigger,'   , ''
UNION ALL SELECT '   connections_touch, BEFORE UPDATE only -- this is an INSERT.'  , ''
UNION ALL SELECT '   The only visible change is that these connections reappear'   , ''
UNION ALL SELECT '   in both members lists and counts.)'                           , ''
UNION ALL SELECT '--- SCOPE ---', ''
UNION ALL SELECT 'rows touching Ian',                (SELECT count(*) FROM payload p WHERE p.requester_uuid=(SELECT uuid FROM ian) OR p.addressee_uuid=(SELECT uuid FROM ian))::text
UNION ALL SELECT 'oldest connection restored',       (SELECT min(created_at) FROM payload)::text
UNION ALL SELECT 'newest connection restored',       (SELECT max(created_at) FROM payload)::text;
