 ...
 *
 *   GET /profile-api/v0/chapters/<slug>   -> { chapter, member_count, is_member, can_post }
 *   GET /profile-api/v0/chapters          -> { chapters: [...] }   (index)
 *
 * PUBLIC — anonymous is a first-class audience (chapters are public + browsable; Ian
 * 2026-07-12). Anonymous simply gets is_member=false, can_post=false.
 *
...
profile_app_json(200, [
    ...
    'is_member'    => $is_member,
    // Read = anyone (public + browsable); post a discussion = members only, and joining is one
    // tap. This is the recommended rule pending Ian's ruling on the open board question.
    'can_post'     => $is_member,
]);
