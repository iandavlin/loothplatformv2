...
function dir_member_display(array $r, array $vArr): ?array
{
    ...
    // exactly as built, so OFF is a byte-identical no-op.
    //
    // ONE SWITCH, deliberately. The merge of backlog 20 briefly landed TWO —
    // this clamp and a predecessor's dir_location_cap_on() env/config cap that
    // ran just above and downgraded $precision before this code was reached.
    // Both defaulted OFF so nothing leaked, but they were NOT equivalent (the
    // other one never applied the structured-labels-only rule, so it left the
    // pin-less rows that hold real street addresses) and with it ON this
    // function's OFF path stopped meaning pre-feature behaviour. Keeper ruled
    // 2026-08-16: dedupe to this one. A dead OFF flag is what gate 49 eats.
