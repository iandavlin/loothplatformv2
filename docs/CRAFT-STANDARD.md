...
> **This table has holes: no row for 19, and none for 22.** Both landed in
> `run-all.sh` without their table row, and rows 20/21/23 were all added against a
> table that stops at 18. Whoever owns 19 and 22 should fill them in — do not
> renumber around a hole.
>
> **Row 23 collided TWICE before it landed**, as "20" and then as "21", while the
> mirror-sync lane sat unmerged and main gained two more gates. Both times the
> resolution was to keep BOTH and renumber your own. The second collision arrived
> through a perfectly **clean auto-merge of this file** — two rows numbered 21, no
> conflict marker, nothing to notice. After any rebase, GREP THE ROSTER for
> duplicate numbers in both this table and `run-all.sh`; a quiet rebase is not
> evidence of a free number.
