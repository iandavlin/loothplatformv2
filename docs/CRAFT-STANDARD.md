...
> ⚠️ **Rows 19 and 22 are missing from this table, not from the suite.** `social-actions-wired-gate.py` (GATE 19/23) and `back-pill-navigates-gate.py` (GATE 22/23) are registered in `run-all.sh` but were never added here, so this table under-reports the suite by **two** and the gap is widening — three lanes have now numbered past it (20, 21, 23). Left for each gate's owner to describe: guessing at another lane's gate is how a table starts lying. **Mint from `run-all.sh`, never from this table's last row.**
