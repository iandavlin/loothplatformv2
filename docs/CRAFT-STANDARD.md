...
| 36 | `tools/gates/anon-dark-contrast-gate.py` | **a logged-out visitor in DARK MODE can read the sign-in/join/sign-up path.** Backlog 21, Ian 2026-08-14: *"the dark mode needs some love for the login stuff"* + *"a ton of instructions and fields not ready for primetime in dark for logged out."* Measures WCAG contrast (text 4.5:1, large text/field borders/icon controls 3:1) on `/wp-login.php`, `/join`, `/lgjoin` and the front page, in BOTH dark paths — **app-dark** (the visitor picked Dark; the nginx boot script force-paints `body{background:#15171a!important}` pre-paint, including on `/wp-login.php`, which has none of this project's CSS) and **os-dark** (`prefers-color-scheme`, since dark here is a resolved app theme, not a media query, so nothing arrives pre-paint) — at desktop and mobile. RED-FIRST, confirmed live before any fix: the login page's three-card skin (`lg-snippets/snippets/86.php`) predated dark mode entirely and stayed white inside a forced-dark page; the mobile "+" compose icon is 1.85:1 (`--lg-sage-d` repoints to a LIGHT colour under dark while its icon's hardcoded-white stroke does not follow); `/join`'s intro paragraph inherited the boot script's `body{color:#e5e7e1!important}` while its own card background never moved, landing near-invisible pale-on-white. Shares its contrast math (`tools/gates/lib/contrast-probe.js`) with the sweep that ranked these surfaces (`tools/preview/dark-anon-sweep.py`) — one probe, so the gate cannot go green on a page the sweep photographed as the worst one. The login-page and `/join` fixes have since landed unflagged (neither surface is member-visible — a logged-in member never sees either in normal use); the "+" icon fix is held pending keeper's ruling on whether a pure dark-mode colour correction needs a flag, since two existing precedents in the same codebase (the sign-in pill's ink-flip, `--lg-follow-on`) ship that exact class unflagged | chrome-dev on 127.0.0.1:9222, `tools/gates/gate-env.sh` resolving a token |
