...
| — | `tools/gates/roundup-forum-items-gate.py` | **§2.2: the roundup absorbs BuddyBoss's "New discussion: X".** Asserts the tracked flag, that BOTH halves exist (the discussion query AND the suppression seam) and — the safety argument — that they are gated by the **same** switch, so "suppressed but not replaced" is unreachable. Executes the pure subject/split functions in isolation to prove the reply-only wording is byte-identical to the pre-§2.2 strings. **HELD OUT of run-all.sh on purpose** while §2.2 is unmerged: it guards a DEPLOYED mu-plugin, so it can say nothing about an unmerged branch and would report CANNOT RUN for every lane. It joins the suite in the commit that merges §2.2 — the same posture follow-digest-gate.py took (row 13) | `php` only; no box, no database |
