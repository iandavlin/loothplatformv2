...
| hub-anon-excerpts `enabled` | platform/config/hub-anon-excerpts.php (box-local override: `.local.php` beside it) | **false** | ON (dev2 local, Ian's look) | OFF | Logged-out hub viewers were served ~440-char excerpts + a teaser reply, so Google satisfied discussion searches on /hub/ and credited the feed over the canonical landing pages (Ian's 8/19 screenshot, #81). ON = anon sees ~150-char teasers, no teaser reply; members unchanged. OFF is byte-identical. Close #81 at live flip per the ritual. |
