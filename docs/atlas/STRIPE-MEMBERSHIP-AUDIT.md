...
> **⚠️ SUPERSEDED 2026-08-08 — the blast radius below is wrong in the dangerous
> direction.** Re-verification a week on (`docs/atlas/STRIPE-PHASE0-FINDINGS.md`)
> found that applying this cleanup as written moves **10 members, not 3** — four of
> them actively-paying patrons who would be wrongly demoted, because a persisted
> `tier=NULL` patreon row shadows the live reader. 1894 does **not** move (also holds
> `manual_admin=looth3`). The shipped fix repairs the four automatically and holds the
> six real movers for Ian's member-by-member ruling — see the findings doc and
> `docs/atlas/STRIPE-BUILD-HANDOFF-2026-08-08.md` §2. The prose below is kept as the
> 7/30 record.
