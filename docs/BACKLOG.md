...
41. DESK LIFECYCLE + MODAL COMPACTION (Ian 8/16 night: "My desk is now really verbose... should go to modals... more mechanical and less hand curated by keeper... completed work still listed on my desk") — three parts: (a) desk rows COMPACT — one line (seat, ask-type, age), full body in the card modal like branches-on-cards; (b) desk item LIFECYCLE, mechanical — an item retires to history when its decision box is answered, when the related work lands in a train, or on explicit dismiss; nothing lingers after completion and keeper never hand-removes; (c) HAND-CURATION AUDIT — inventory everything keeper still edits by hand on the board's stores (status stamps in this file above all) and propose mechanical derivation (train-landing commit trailers → item status is the likely shape); (d) THE DONE-LEDGER (Ian 8/16: "do we have a file to move completed work to as a history or are we relying on git as a record" — answer was: git only) — docs/DONE.md, append-only, one line per landed item (number · title · landed SHA/train · Ian-verified date · live date), written MECHANICALLY at train landing, never by keeper hand; completed lines MOVE there out of this file; the board's history view renders it; git remains the forensic depth. OWNED: stripe seat (board programme), QUEUED directly after its live-billing preflight.

40. SEARCH CONSOLE COVERAGE TRIAGE (Google's export 8/16 after Ian's sitemap resubmit; data through Aug 9, PRE-dates today's canonicals) — baseline: ~3,801 indexed, 4.48M not-indexed of which 1.27M robots-blocked hub-filter permutations (the block WORKING — infinite faceted space, by design), 97K "duplicate without user-selected canonical" (today's hub canonicals address one class; the rest needs a sweep), 2,393 pages 403ing to Google, 16 indexed-though-blocked. Export unpacked at scratchpad gsc/; ZIP at ~/.
...
39. WIP BOARD: BRANCHES ON CARDS, in the modal (Ian 8/16: "add branches attached to cards in the modal? So I can track branches better") — each board card's modal lists the branches carrying that item's work (branch name, tip, merged-or-not), so branch state is trackable from the board itself. Needs an explicit card→branch link in the committer's write shape (v2 item_add/item_promote is the natural home) plus the modal render. OWNED: stripe seat (board programme, joins the parity roadmap); to relay at its next wake.
