...
29. WIP BOARD (Ian 8/15/morning: dev-gated webpage rendering the backlog as an interactive board) — BACKLOG.md renders as cards; adding from chat or page both COMMIT to this file (truth-list preserved, git-audited); cards carry mockup links, registered branches, notes. Rails from our own workflow: decision queue (rulings waiting on Ian), look queue (phone-pass items w/ preview URLs), flag panel (dev2 vs live), riding-the-next-paste view, live lane rail, gate ledger. MOCK-FIRST (Ian ruled, decision box). OWNED: stripe-membership seat, next after the dress rehearsal.
