...
40. SEARCH CONSOLE COVERAGE TRIAGE (Google's export 8/16 after Ian's sitemap resubmit; data through Aug 9, PRE-dates today's canonicals) — baseline: ~3,801 indexed, 4.48M not-indexed of which 1.27M robots-blocked hub-filter permutations (the block WORKING — infinite faceted space, by design), 97K "duplicate without user-selected canonical" (today's hub canonicals address one class; the rest needs a sweep), 2,393 pages 403ing to Google (check whether bot protection challenges Googlebot or gated pages are publicly linked — the one new lead), 16 indexed-though-blocked. Charter shape: contain the filter space (nofollow/parameter handling so crawl budget goes to real pages), duplicate-canonical sweep, 403 investigation. Export unpacked at scratchpad gsc/; ZIP at ~/. UNOWNED — next free seat.
