...
4.2 Logged-out mobile bottom dash: "+" implies you can post → anon-mobile-dash (fix BUILT, flag off; awaiting Ian's frames look at /footer-mockups/anon-mobile-dash/ — surfaced to his desk 8/16 — then keeper merge)
4.1 Recap: the mobile bell's 700ms mark-all-read empties the recap → recap-read-timer (fix BUILT; in keeper's merge queue)
4.0 Weekly digest: discussion images missing → ROOT-CAUSED 8/16 night (Ian hit it composing issue 72698): the digest scans the WP copy for inline <img>, but composer-made posts store images MIRROR-side only — proven on three live topics (72697/72647/72559, zero inline img). The parked digest-images branch (bp_media, 740 behind) is partial prior art with a self-declared gap. REBUILD chartered to guitardle-fairness seat 8/16 (measure the real store, resolve from truth, red-first vs a live-shaped fixture)
...
38. AUTHOR-FILTER HEADER SOMETIMES MISSING (Ian 8/16, two live screenshots: "the author search is some times not returning the header like it is for patrick") — hub ?author= view shows the green author banner for Patrick Niedermeyer but NOT for Rick Liftig (display name with dots/possible truncation in the param); posts filter correctly in both cases, only the banner drops. Suspect the banner's member lookup and the post filter are two different matchers and the banner one is fragile on awkward names — unverified. Repro needs a dev2 member with an awkward display name (live data ≠ dev2 data). Doubly urgent: the banner is the landing experience of item 27's new icon. OWNED: featured-members seat, relayed 8/16 16:2x.

**P1 — wanted now / deploy-blocking**
47. UNREACHABLE REMOTE at /home/ubuntu/projects — DATED **2026-08-20**, owner keeper (Ian's yes 8/18 via the planning chat). The projects repo's origin uses ssh alias `github-looth`, which has NO entry in ssh config, so its 8 worktrees' commits exist on this disk ONLY (disk-audit 8/16 finding; keeper-repo itself verified clean). Work: run the worktree + push-state survey there, fix the alias, push everything, report what was actually at risk. The one open item that can lose work.
...
8. Front page: latest weekly email for logged-out users → guitardle-fairness. RULED 8/15 (Ian, after the mock: "build it and let me see it on dev2" — he skipped the A/B pick deliberately). BUILT: Option A, the latest SENT issue's own stored sections rendered as the front page's own cards, under the welcome video and above the featured member, member-only items shown WITH their padlock. Flag platform/config/weekly-front.php OFF in the repo, ON on dev2 for his look. Reads LG_WD_Query::build_payload_from_issue via a new nopriv feed — the same resolver the email uses, so nothing is re-derived. Gate 54, red-first on 11 mutations. The email-window variant (Option B) is drawn, costed and NOT built — one ruling away. Mock + decision record: /footer-mockups/weekly-front/. Two live defects found on the way, both Ian's to fix, neither blocking: the public sign-up page is behind a login wall on LIVE (missing from the 67-entry BuddyBoss public-content allow-list), and the email builder dates every re-render by the clock so dev2 serves the July 13 issue as "Week of August 15"
9. Shop Layout Planner — apps index page ONLY (standalone render SHIPPED to live) — UNOWNED
18. FEATURED MEMBERS — opt-in consent + admin dash + auto-publish (Ian 8/11, "fairly soon type of thing") → featured-members. ✅ ALL THREE PIECES BUILT, WIRED, AND IN ACTIVE USE — inventoried 2026-08-16 after Ian himself asked "isn't it most of the way built?" and was right; nothing was missing to mock. (1) TICKBOX: profile-app/web/u.php's Option-B card (Ian's ruling, docs/IAN-RULINGS-2026-08-14.md item 6, after decide.html), flag platform/config/featured-members.php enabled=true since 8/15. (2) ADMIN DASH: lg-layout-v2/src/FeaturedMemberDash.php (pool + featured history, who/when), wired via Plugin.php, live at wp-admin/admin.php?page=lg-featured-member. (3) AUTO-PUBLISH: archive-poc/api/v0/_config.php is the one write door — a dash selection writes config.json's featured_member AND logs discovery.featured_history in the same call, one-at-a-time enforced by a partial unique index. PROVEN REAL, not just built: featured_history already holds 2 real rows (Rick Liftig → Ian Davlin, chosen_by=iandavlin both times) — Ian has used the real dash; keeper corroborated the table independently the same night. Gate 39, 11 assertions green (schema constraints, completeness-vs-SQL-report parity, flag-off no-op, no admin-override consent bypass, both-theme CSS gating). Backlog 34 rides in the same file (see below). The line sat UNOWNED for a day after the build — the stale-ledger failure item 41's done-ledger automates away.
...
S2 ~~/footer-mockups/ anon-readable on live~~ — CLOSED 8/16: Ian ran the `sudo rm`, public URL verified 404. Open since May 18; the 8/16 desk audit surfaced it.
...
20. DIRECTORY SHOWS CITY/STATE ONLY (Ian 8/12, member-safety) — **BUILT, MERGED TO main, GATED, flag still OFF.** Was reading UNOWNED after being fully done since 8/15 — two seats independently built it same night (handoffs/2026-08-15-directory-location.md), reconciled to one fix, merged. `profile-app/api/v0/directory-members.php`'s `dir_member_display()` clamps `street`→`city` and takes the label from `Block::listLocationText()` (structured-only, never falls back to the row's literal text — the part that closes the actual leak: 2 pin-less rows on live hold a full street address with no city/region columns). Also caps the admin/owner audience, who unconditionally got `street` for EVERY member before this — reframed the bug from "a 7-row public leak" to "every directory page shows 15-16 addresses to any admin." Flag `platform/config/directory-location.php` → `coarsen_list_location`, re-verified GREEN just now: gate 44 (72 assertions) + the cap gate (5 assertions, red-first against Luke's own fixture + a second real one) both fresh-run clean. Shops question already ruled by Ian 8/15: no business_name exemption, truncate everyone uniformly. **THE LEAK IS STILL LIVE while the flag is OFF** — this is proven ready, needs Ian to look at the real running preview and approve the flip, same shape as backlog 18's tickbox. Not done: the affected rows' `location_text` column itself still stores the raw address (rendering-safe once ON, but the store is unfixed — a future geocode/redaction pass, not blocking the flip).
...
22. GUITARDLE: extra chances by switching devices (Ian 8/14: "fixing the guitardle giving more chances on different devices") — UNOWNED. Today the daily attempt count lives on the DEVICE, so a new device/incognito window = a fresh set of tries — and the Weekly Top 5 has claimable spots, so device-hopping is a fairness hole, not just a quirk. Fix shape: attempts tracked server-side against the MEMBER ACCOUNT for logged-in players (one daily allowance no matter the device); decide the anon story (anon can play but can't claim spots? or anon stays device-tracked and only members are eligible for the Top 5 — likely Ian's call). Audit where the game stores state before building.
...
27. AUTHOR ARCHIVE BUTTON in the hub header links (Ian 8/15: "We seem to not have a button for the user archive in the hub. Which would be the author search... basically in the header where the links go") — from a member profile, a header link opens the hub filtered to that author's posts. ⚠️ RE-RULED 8/16 from the mock: PILL REJECTED ("The author archive is not what I am looking for"). It is now ONE MORE ICON in the profile's existing icon palette (the socials row), styled like its neighbours, linking to the hub's adv-search author filter (the ?author= filtered view). OWNED: featured-members seat, relayed 8/16 16:2x.
...
29. WIP BOARD — the board itself (Ian 8/15: "a practical board that I would like to use to great effect. Maybe even transition away from vs.") — RUNNING at /wip-board.php (dev-gated), accordion v1.5: projects as drawers, done items self-clear, per-item modals, sentinel-fed team lights. OWNED: stripe-membership seat, resumes right after the stripe whitelist build. Ian's open fixes in his order: (a) row-to-modal MISMATCH (clicking an item opens the wrong details) — FIRST; (b) copy-paste bridge (a section Ian can copy from the board back into VS chat, since keeper can't sit in the page); (c) general chat + per-item chat threads, both bridging to keeper; (d) his desk strip (docs/IAN-DESK.md) rendered on top; (e) drag-to-rank within a project; (f) since-you-last-looked digest + capacity strip refinements. Adding from chat or page both COMMIT to this file (truth-list preserved, git-audited). Ian 8/15 evening: the board must list ITSELF — this entry is that. 8/16: write layer LIVE (Ian's first real reorders committed as ian-via-board); watch-only lane terminals shipped per team row; NEW CHARTER (Ian 8/16: "I would like to be able to interact with the lanes through the workboard") — per-lane message threads through the fenced committer + keeper relay via lane-say, replies flow back; stripe seat designing the relay before building.
30. HISTORICAL MEDIA ORPHAN AUDIT (8/15, surfaced by the compose orphan measurement) — 4,879 of 7,397 attachments (66%) have no parent post, predating compose; nothing has ever swept them. AUDIT FIRST: "unattached" is not "unused" in WP (files embed in content without post_parent), so phase 1 is a reference-scan classifying truly-dead vs embedded-but-unparented, report sizes, NO deletion without Ian's ruling on the classified list. UNOWNED.
31. BUSINESS PROFILES for members ★ vision (Ian 8/15, from the shops ruling: "We are eventually going to add a business profile for users") — a declared-business layer on member profiles; until it exists, location truncation applies to everyone uniformly and members adjust their own location text. UNOWNED, vision tier.
34. FEATURED CARD COPY (Ian 8/15 night, screenshot): "100% COMPLETE — CARD READY" means nothing to a member — replace the completeness meter's framing with encouragement, his words: "more complete profiles have a higher chance of becoming featured. Or something along those lines." Small copy change on the opt-in card. ✅ SHIPPED, featured-members seat — profile-app/web/u.php's meter now leads every state with "More complete profiles have a better chance of being featured," the old "card ready" framing is gone from the surface (card_ready flag stays in Completeness.php, only the wording changed). Live in the same build as item 18.
35. GUITARDLE REPEAT — DUPLICATE PHRASE TEXT (Ian caught it on live 8/15): "DAN ERLEWINE" lived under BOTH phrase ids 180 and 233, so members got the same puzzle on June 23 (day 12) and again Aug 15 (day 65). The no-repeat shuffle itself is sound (285 unique ids, first wrap 2027); the dupe TEXT defeated it. **DONE 8/15 on branch `guitardle-phrase-dupe`, MERGED train 3 8/16 — NOT yet on live.** id 233 retexted to "notched straightedge"; id 180 deliberately left alone because it was THAT DAY'S LIVE PUZZLE and the saved-game snapshot keys on phrase id, not text, so retexting it would have swapped the answer under members mid-game. 233 was 53 days past on the member track and still in the FUTURE for the logged-out track (+142: it had consumed only indexes 142..207 and does not reach index 12 until day 155), so neither track now sees the phrase twice. Zero index/sequence changes, no row removed, both assets still served. Gate 60 + 13-mutation red-first (renumbered from the lane's working 55 at keeper mint). **LIVE still carries the dupe** — it reaches members only via Ian's next paste of the two assets.

    **ALL THREE CHARTER PIECES DONE 8/16, two seats:**
    - **403 investigation — ANSWERED (stripe-membership seat)**, docs/BACKLOG-40-GOOGLEBOT-403S.md: origin has never 403'd Googlebot (both live access logs checked) but DOES 403 other agents on gated API paths, so the 2,393 are issued at the Cloudflare edge before reaching origin — likely Bot Fight Mode. Fix is in the Cloudflare dashboard, Ian's to flip. Second finding riding along: Googlebot is crawling and 404ing on test-account profiles (smoketest1/smoketestpublic/stink_305efb/tst-staple).
    - **Duplicate-canonical sweep + filter-space containment + the standing gate — DONE (featured-members seat)**, pushed through bfe6706: `profile-app/web/p.php` now carries the same self-referencing canonical/description/OG pattern as `u.php` (the one real gap found — WP CPT permalinks, archive-poc's own pages, and the Hub's own canonical were all already clean). Hub filter rail (`_filter-rail.php`) now sends `rel="nofollow"` on every link that ADDS a facet, leaving reset/clear/un-filter links followable so Google can navigate back toward the clean state — the robots block stays as backstop, this stops the bleed at the source. `tools/gates/sitemap-anon-open-gate.py` built and run for real against live sitemap data (content-presence discriminator per Ian's promised standing gate, not a login-string check): 17/18 sampled URLs green with real DB ground truth, the 18th a genuine test-fixture leak (`/u/ut-test-alice`, `users` id 1999 — a THIRD independent instance of the same class stripe-membership found). Awaiting a gate number from keeper to land in run-all.sh; not yet a merge-blocking gate.


46. BOARD STRIP (Ian 8/16 night, the board conversation's ruling: "Lets start from the board. Lets strip everything away except for the headings and the accordion items.") — the board reduces to section headings + items as collapsible accordion rows, rendered from the md file directly (the mirror architecture: file is the only store, no parser-invented structure). Chat/desk/threads/badges/terminals strip OFF the board page — their underlying stores (msg board, doorbell, relay) keep running untouched, surfaces to be reconsidered separately if wanted. Built on board-mirror.php's bones; wip-board.php untouched until Ian blesses the replacement.
45. DISK AUDIT (Ian 8/16 night: "the disc is at 93" + "do we have a lane that can do an audit?") — keeper swept 8 clean retired worktrees (93→90%); the seat triages the 35 dirty/unregistered holdouts (salvage-to-branch / delete / ask-Ian, nothing unrecoverable deleted), drills /var/www 3.1G + .local 1.4G, and inventories the 2.7G of archives (dev1-import, dev26-archive, backups) into an Ian decision table — archives untouched without his word. OWNED: stripe seat (fresh respin), in flight 8/16.
44. COMPOSER REDESIGN — WRITE FIRST, CATEGORIZE LAST (Ian 8/16 night, thinking-aloud→ruled: the Where step "a pain point for posting"... "maybe we add it at the end as optional. bonus points"). Category step moves to the END of the flow, OPTIONAL, encouragement-framed, built on the CUSTOM TAXONOMY (his clarification — taxo chips, not the forum list; forums become plumbing, General default). Evidence: the busy General sub-forum is the #1 destination for new topics. After-the-fact categorization: keeper's live watch + supervised suggestions to Ian's desk. OWNED: frontend-compose, mock-first, in flight 8/16.
43. HUB AUTHOR BANNER — CLIENT PATH RENDERS A LESSER BANNER (Ian 8/16 night, live screenshot: Max's banner letter-avatar, no View-profile link) — the SERVER render is complete (real avatar, measured); some in-page path (suggest or swap) builds a degraded banner. Gate 64 asserted named-and-present, never avatar+link PARITY with the server render — the assertion gap closes with the fix. Third member of the banner family; converges on carry-the-member-ID. OWNED: featured-members, in flight 8/16.
42. LOCAL LOOTHS REWORK ★ vision (Ian 8/16 night: "We are going to change the way local looths works with their own pages etc. It can leave the main area for now.") — chapters get their own pages, out of the plain-forum model. RULED at the same time: NO interim structural work — the ten top-level chapter forums stay as they are (no Local Looths container, no picker re-grouping code; both explicitly stood down pending this rework). The two-Generals naming collision and the Middle Tennessee duplicate (58440/58442) fold into this rework's scope rather than getting interim fixes. UNOWNED, vision tier.
37. PAIRED-TOKEN PALETTE — dark mode's structural fix (Ian ruled ADOPT, 8/16, decision box; mock-first per his ask "is that worth a mockup?"). Root cause of five dark bites: contrast is a property of ink-on-background PAIRS but our code writes single colors, so every seam risks a half-themed combination. Build: (a) SWATCH SHEET first — every semantic pair in both themes side by side, measured ratios printed, dev-gated, Ian approves colors from the picture; (b) then one shared tokens file of the approved pairs; (c) new surfaces MUST consume pairs from it, old surfaces migrate when touched (CRAFT-STANDARD "both themes from birth", 8/16). OWNED: dark-anon-sweep seat, after their board contrast pass.
36. GUITARDLE LETTER-STATE + CROSS-DEVICE MOVE LOSS (Ian playing, 8/16 morning) — three symptoms from his own round: desktop picker keeps only HITS lit (misses lost); mobile picker right but correct letters not painted into the puzzle word; a desktop refresh lights ALL letters yet DROPS recent guesses made from mobile and even an earlier desktop one — the shared play record looks clobbered rather than appended, and moves are scoring input, so a lost move is a fairness bug. OWNED: guitardle-fairness seat, TOP JOB — blocks Ian's cheat-proofing approval and therefore the live flip. Repro contract: one member, two devices, interleaved guesses, refresh each side, identical complete state both ends; invariant gated red-first.
33. LIVE lg-wp-cron UNIT DRIFT (surfaced by paste 2's lg-deploy WARN, 8/15) — repo copies of lg-wp-cron.service/.timer differ from what's installed on live. AUDIT WHICH SIDE IS RIGHT before anyone runs the suggested cp+daemon-reload: diff repo vs installed via live-ro, check unit mtimes against git history for an unshipped repo update vs an intentional live-only tweak. Small, read-only until ruled. UNOWNED.
...
## 4.4 + 4.3 Mobile profile tray: DMs and the 3-dots menu — FIXED, ON, LIVE (Ian verified dev2 8/9; shipped in the 8/15 pastes)
