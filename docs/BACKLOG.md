...
3.8 Mobile/PWA post → hub BACK NAV → ✅ BUILT (Ian ruled option D, 8/9): slim 32px "← Hub" pill, top-centre, hides on scroll-down and returns on scroll-up. webroot/bottom-nav.js beside the tray Back it promotes (no new webroot file, so no symlink coupling), flag platform/config/back-pill.php OFF by default, gate 22 (10 assertions, all reddened). Verified on real /hub/ paths: rest visible → scroll down hidden → scroll up back, and a hit-tested tap LANDS ON /hub/. Awaiting merge + Ian tapping it on the serve. Frames: /footer-mockups/post-back-nav/
