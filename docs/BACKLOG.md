...
18. FEATURED MEMBERS — ✅ BUILT, WIRED, IN ACTIVE USE (verified independently 8/16 night: discovery.featured_history shows Ian himself featured Rick Liftig 8/15 then himself 8/16 via the dash at wp-admin → Featured Member; gate 39 green 11/11 both themes; tickbox flag ON since 8/15 by his own "flip when landed"). The line below simply never retired after the 8/14-15 build — the exact stale-ledger failure item 41's done-ledger automates away. ORIGINAL ASK (Ian 8/11): opt-in consent + admin dash + auto-publish. THE FLOW: (1) member ticks a TICKBOX in their profile = explicit agreement "to be included as a potential featured member" → joins the SELECTABLE pool. Consent is opt-in, OFF by default, EXPLICIT — never inferred (email-project consent lesson); per-member flag (usermeta/profile-app field). (2) an ADMIN DASH shows the selectable pool + tracks featured HISTORY (who + when). (3) admin SELECTS a member in the dash → they are AUTOMATICALLY added to the front page (no separate publish step; deselect removes them). Front page reads only opted-in + currently-selected. Scope: profile tickbox is member-facing (flag it); admin dash is admin-only; front-page block is public. Mock the tickbox + dash + a sample front-page card for Ian BEFORE build
