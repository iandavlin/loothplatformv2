...
3.8 Mobile/PWA: post → hub BACK NAV — ✅ RULED 8/9 (decision box): build OPTION D, the sticky '← Hub' pill (~32px, hides on scroll-down, reappears on scroll-up). Ian liked B first, then asked for low-profile+sticky and D won. → mobile-bugs BUILDING, flag OFF, red-first nav gate, layout-independent (does NOT wait on hub-landing). A/B/C not built. Mock: https://dev2.loothgroup.com/footer-mockups/post-back-nav/
