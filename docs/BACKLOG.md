...
3.8 Mobile/PWA: post → hub BACK NAV must be EXPOSED — mobile-bugs, MOCKUPS UP but PARKED behind the new Hub landing (Ian 8/9: the frames carry the legacy forum layout he wants gone). Option A restyles the very header the landing replaces, so A must be re-shot on the new layout; Option B (bottom-bar Hub item) touches no page chrome and is layout-independent. Re-shoot + republish the same URL once the landing merges+flips. Page banner says so. Original ask: Ian 8/9: "on mobile and pwa we need some kind of back nav to the hub once you click through to the post. there is one in the nav tab but it should be exposed. Need mockups." House rule applies: draw it behind the dev gate (recommendation + at most one alternative, side by side), hand him a URL; no code before his pick
