...
3.8 Mobile/PWA: post → hub BACK NAV — ✅ RULED + REFINED 8/9: build a HYBRID = option C's POSITION (floating '← Hub' chip, LOWER-LEFT thumb corner) + option D's BEHAVIOUR (appear-on-scroll, hidden at top). Ian's words: "can we have the back button be the appear on scroll hub button but have it in the lower left?". → mobile-bugs BUILDING, flag OFF, red-first nav gate, layout-independent. Mock re-shoot: hidden-at-top AND revealed-on-scroll. https://dev2.loothgroup.com/footer-mockups/post-back-nav/
