...
/**
 * Lean the comments-frame view (the asset-weight win).
 *
 * On ?lg_comments=1 the template below renders its OWN self-contained markup +
 * styling (brand palette, comment thread, native form) — it needs no theme CSS.
 * But wp_head() still prints whatever every active plugin/theme enqueued
 * globally: on a non-v2-managed singular (e.g. a /stream/ post) that is the full
 * BuddyBoss-theme + BB-platform(+pro) + Elementor + Dynamic-Content + Search-
 * Filter stack — ~50 stylesheets / ~120 KB of render-blocking dead weight for a
 * comment thread. Dequeue everything except the tiny allowlist the native form
 * actually needs.
 *
 * Mirrors lg-layout-v2/src/Isolate.php's dequeue pass, but scoped to this view
 * and more aggressive (the template needs zero external CSS). Runs at
 * PHP_INT_MAX to win the timing race against late enqueuers. Order-independent
 * w.r.t. Isolate (which also runs at PHP_INT_MAX on managed CPTs): both only
 * dequeue, so whichever runs second no-ops on the already-pruned registry.
 *
 * NOTE: this only strips *enqueued* assets. It cannot touch the ~1.2s PHP-boot
 * floor (plugins fully init before template_redirect) nor the handful of inline
 * <style>/<script> blocks the theme/customizer echo directly on wp_head/wp_footer
 * (e.g. buddyboss_theme-style, wp-custom-css) — those are assessed separately.
 */
add_action('wp_enqueue_scripts', function () {
    if (empty($_GET['lg_comments']) || !is_singular()) return;

    /* Styles: drop them all — the template is fully self-contained. */
    global $wp_styles;
    ...
    /* Scripts: keep only jQuery (the theme's "disable submit until typed" inline
       comment script depends on it) and core comment-reply (threaded inline
       reply). Everything else goes. */
    $keep = array_flip(['jquery', 'jquery-core', 'jquery-migrate', 'comment-reply']);
    global $wp_scripts;
    if ($wp_scripts instanceof WP_Scripts) {
        foreach (array_keys($wp_scripts->registered) as $handle) {
            if (isset($keep[$handle])) continue;
