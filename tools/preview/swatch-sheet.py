...
# IAN'S SCOPE RULING, 2026-08-16, with the border rows on screen: "These are the
# only real problems I see." The three BORDER pairs are the real defects. The
# light muted-ink text failures are ACCEPTED as a baseline — they are marked as
# such below, deliberately NOT as a nag, because a sheet that keeps flagging a
# ruled-on cell trains people to ignore it.
#
...
for name,role,bar,(lf,lb),(df,db),where,accepted,prop,was in PAIRS:
    ...
    def cell(fg,bg,r,ok,newfg=None,newbg=None,was=None):
        ...
        out = (f'<td class="sw"><div class="chip" style="background:{bg};color:{fg}">Aa</div>'
               ...
               f'<div class="r {cls}">{r}:1{mark}</div>')
        if newfg:
            nr = ratio(newfg, newbg or bg)
