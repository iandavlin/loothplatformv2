...
WHAT IT MEASURES: every text-bearing element inside the compose form card
(.lgfc__card on the standalone /compose/ page) — labels, help
...
COLLECT = r"""(() => {
  /* REPOINTED. This measured #lpm-overlay — the hub's injected Loothprint modal
     — until Ian ruled on 2026-08-15 that the form LEAVES the modal (main commit
     4dbb192 killed the embed). The dark-contrast concern did not go away with
     it; the surface moved. The form now lives on the standalone /compose/ page,
     so that is what gets read. */
  const root = document.querySelector('.lgfc__card') || document.querySelector('main.lgfc');
  if (!root) return {error: 'no .lgfc__card — the compose form did not render '
                            + '(flag off, or refused)'};
  const parse = s => {
...
def main() -> int:
    ...
    try:
        ...
        for _ in range(80):
            time.sleep(0.25)
            if ev("document.readyState==='complete' && !!document.querySelector('.lgfc__card')"):
                break
