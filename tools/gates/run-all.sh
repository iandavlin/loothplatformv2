...
echo
echo "=== GATE 3/6: infra-sec gate (cookie auth / source disclosure / cdp) ==="
bash "$(dirname "$0")/infra-sec-gate.sh" || red=1
echo
echo "=== GATE 4/6: hub paragraph-collapse (content_html keeps its breaks) ==="
bash "$(dirname "$0")/hub-content-paragraph-gate.sh" || red=1
echo
echo "=== GATE 5/6: looth-auth-issue (non-REST mint bounce; recurs every DB reload) ==="
bash "$(dirname "$0")/looth-auth-issue-gate.sh" || red=1
echo
echo "=== GATE 6/6: event-date TZ (a UTC 'today' must not judge a site-local date) ==="
