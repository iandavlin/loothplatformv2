...
echo
echo "=== GATE 15/15: placeholder slug HEALS, and flag OFF is a real no-op ==="
# 146 live members are stranded at /u/patreon_<id> because ensureSlug's
# `slug IS NULL OR slug = ''` guard reads a placeholder as a real handle and never
# upgrades it once the member's name arrives (docs/atlas/SLUG-PLACEHOLDER-RECURRENCE.md).
# Runs BOTH flag states: OFF must be a no-op *with the machinery proven live*, ON must heal,
# park the old handle for the 301, and REFUSE the four human-ruling cases (contested bare
# first name, duplicate display_name, held handle, no honest derivation).
# Number minted from MAIN's count (14), not the branch's.
run "slug-placeholder-heal" bash "$(dirname "$0")/slug-placeholder-heal-gate.sh"
echo
