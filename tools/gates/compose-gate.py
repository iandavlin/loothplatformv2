...
def main():
    ...
    try:
        ...
        # ---- 9. THE TWO COMPOSERS ARE SEPARATE MODALS (Ian 2026-08-15).
        #
        # "Cant it just replace the discussion modal ?" — so the Loothprint form
        # lives in its OWN overlay, and the toggle closes one and opens the other.
        #
        # WHAT THIS IS DEFENDING. Ian's screenshot: with the Loothprint tab
        # active, the discussion wizard's furniture — step rail, forum picker,
        # Cancel/Next — was still on screen below the embedded form.
        # ntmSetState('authed') sets ntmForm.hidden = false UNCONDITIONALLY,
        # knowing nothing about the active type, and the auth probe resolves 2-4s
        # AFTER the composer opens. Tap Loothprint inside that window and the
        # wizard is re-shown under the form. Measured on the live serve: the tab
        # read "loothprint" while form, rail, footer and forum list were ALL
        # visible at t+4s. A RACE — which is why a check that waited before
        # clicking called it fine, and why "the form is hidden" would have passed
        # while Ian was looking at the bug.
        #
        # So the assertion is STRUCTURAL and cannot be raced: the discussion
        # overlay must contain NO compose surface of its own. A surface that is
        # not inside that modal cannot be stacked under by a timer.
        #
        # AS AN ALLOWED MEMBER, on the PREVIEW: with the flag off no composer is
        # emitted at all and an anon fetch sees none either, so "no iframe" would
        # be true of a page with no composer — the vacuous-absence trap. No
        # toggle ⇒ SKIPPED, never a pass.
        #
        # RED-FIRST: put any compose frame back inside #ntm-overlay and this
        # fails.
        ...
        if hub_code != 200:
            ...
            # The discussion overlay, from its id to the start of the next overlay.
            i = hub_body.find('id="ntm-overlay"')
            j = hub_body.find('id="lpm-overlay"')
            disc = hub_body[i:j] if (i != -1 and j > i) else hub_body[i:i + 40000]
            if "<iframe" in disc or "lpm-frame" in disc or "ntm-lpframe" in disc:
                findings.append(
                    "[9] the DISCUSSION modal still contains a compose surface "
                    "(an iframe inside #ntm-overlay). That is the stacked-furniture "
                    "defect: ntmSetState('authed') re-shows the wizard under it on "
                    "a timer it does not coordinate with.")
            elif j == -1:
                findings.append(
                    "[9] no dedicated Loothprint overlay (#lpm-overlay) — the two "
                    "composers are not separate modals.")
            else:
                print("  [9] two SEPARATE overlays and no compose surface inside "
                      "the discussion modal — they cannot be open together")
