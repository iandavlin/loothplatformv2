...
- **Comp timers (#183) live in `Membership/CompExpiry.php`, and `Arbiter::sync` is STILL the only role writer.** The sweep decides WHO has lapsed; the Arbiter decides WHAT they become. Do not add a second writer — the old `lg-looth4-expiry` plugin wrote `wp_capabilities` directly and that is the root of the double-role bug the Arbiter's looth4 comment describes. It is gone from live (measured 8/21: absent from `active_plugins`, `recently_activated` empty, no cron event) and must not be resurrected.
- **`looth4_expires_at` is stored in UTC, and both boxes run `America/New_York`.** Reading it in the site zone puts every expiry FOUR HOURS LATE — that was a real bug fixed in #183. Anything reading or writing it goes through `CompStanding::parse` / `gmdate`. Two proofs the value is UTC: the old plugin's own source note, and both live rows' minute-of-day matching their UTC registration.
- **An expired comp is NOT flattened to looth1.** The role comes off and normal arbitration runs, so a comp who also pays lands on their real paid tier; looth1 is only the floor when no paying opinion exists anywhere. A member left with no looth role at all is a bug, not a demotion.
- **Comp expiry is OFF by default and `effective_from` is empty** (`platform/config/comp-expiry.php`, box-local `.local.php` overlay). Empty cutover = detect-and-report: the sweep runs, logs, and demotes nobody. Only a timer that ran out AT OR AFTER the cutover is enforced, which is what holds users 1829 and 1865 — Ian ruled those two are left alone.
