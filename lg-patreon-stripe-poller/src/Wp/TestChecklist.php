    ...
    private const SECTIONS = [
        ...
        'admin' => [
            ...
            'items' => [
                ...
                'ad-pages-sync'  => [ 'desc' => 'In LG Member Sync, click "Re-create / sync membership pages".',                                                      'expect' => 'Missing pages get created with their shortcodes. BuddyBoss public-content allowlist updates.' ],
                'ad-mosaic'      => [ 'desc' => 'In LG Member Sync (welcome mosaic), pick attachments and save.',                                                     'expect' => 'lgms_welcome_mosaic_ids option saved. Welcome email mosaic now shows those images.' ],
                'ad-loothalong'  => [ 'desc' => 'In LG Member Sync, set the Loothalong Zoom URL and save.',                                                           'expect' => 'lgms_guide_loothalong_url option saved. /membership-guide/ for members shows the live link.' ],
        ...
        'cron' => [
            ...
            'audience' => 'admins',
            'items' => [
                'cr-tick-manual'    => [ 'desc' => 'Trigger Tick::run via /run-now (or wp cron event run lgms_poll_tick).',                                                       'expect' => 'the tick log (wp-content/uploads/lg-logs/tick.log — exact path from \\LGMS\\Log::status()) shows: tick start → stripe poll → expiry sweep → reconcile-pending → sync sweep ok=N errors=0.' ],
                'cr-tick-lock'      => [ 'desc' => 'Fire two concurrent /run-now calls.',                                                                                       'expect' => 'the tick log shows one "tick start" and one "tick SKIPPED: another tick is already running".' ],
